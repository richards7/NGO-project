...
export class AnalyticsService {
  async getDashboardSummary(campId?: string) {
    ...
    const [
      ...
      db.camp.count({ where: { status: "Completed" } }),
      db.camp.count(),
      db.prescription.count(),
      db.medicineTransaction.count({ where: { type: "DISPENSED" } }),
      db.doctorNote.count(),
      db.patient.count({ where: { status: "Completed" } }),
      db.user.count({ where: { role: { name: "doctor" } } }),
      db.user.count({ where: { role: { name: "registration" } } }),
      db.user.count({ where: { role: { name: "pharmacy" } } }),
    ]);
    ...
    return {
      ...
      totalRegistrationStaff: registrationCount,
      totalPharmacyStaff: pharmacyCount,
      totalStock,
      lowStockCount,
    };
  }
